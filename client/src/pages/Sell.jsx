...
export default function Sell() {
  ...
  const handleImageUpload = (e) => {
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      Promise.all(promises)
        ...
        .catch((err) => {
          setImageError("Image should be less than 2mb");
        });
    } else {
      setImageError("You can upload only 6 image");
      setUploading(false);
    }
  };
  ...
  return (
    <div>
      {token ? (
        <main className="py-2 px-10 m-6 rounded-lg  max-w-4xl mx-auto bg-white">
        ...
        <p className="text-center p-10  text-xl sm:text-2xl font-semibold">
          Please Sign In to Sell Your Home
        </p>
      )}
    </div>
  );
